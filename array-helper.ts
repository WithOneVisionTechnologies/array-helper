  ...
  public static uniqBy<T>(arr: T[], key?: string): T[] {
    if (key === undefined || key === null || key === "") {
      return arr.filter((value, index, arr) =>
        arr.findIndex((x) => value === x) === index
      );
    }

    return arr.filter(
      (value, index, arr) =>
        arr.findIndex((x) => value[key as keyof T] === x[key as keyof T]) ===
          index,
    );
  }
    ...
    return arr.sort((a: any, b: any) => {
      ...
      while (result === 0 && i < length) {
        ...
        result = direction === "desc"
          ? b[key as keyof T].toString().localeCompare(
            a[key as keyof T].toString(),
          )
          : a[key as keyof T].toString().localeCompare(
            b[key as keyof T].toString(),
          );
        i++;
      }
